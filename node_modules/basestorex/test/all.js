...
test('ram-based basestore, different get options', async t => {
...
test('ram-based basestore, simple replication', async t => {
  const store1 = await create(ram)
...
test('ram-based basestore, replicating with different default keys', async t => {
  const store1 = await create(ram)
  const store2 = await create(ram)
  const core1 = store1.default()
  const core2 = store1.get()
...
test('ram-based basestore, sparse replication', async t => {
...
test('ram-based basestore, sparse replication with different default keys', async t => {
  const store1 = await create(ram, { sparse: true })
  const store2 = await create(ram, { sparse: true })
  const core1 = store1.default()
  var core3 = null
...
test('raf-based basestore, simple replication', async t => {
...
test('raf-based basestore, close and reopen', async t => {
...
test('raf-based basestore, close and reopen with keypair option', async t => {
  var store = await create('test-store')
...
test('top-level basestore replicates all opened bases', async t => {
  const store1 = await create(ram)
  const store2 = await create(ram)
  ...
  await runAll([
    ...
    cb => core2.ready(cb),
    cb => {
      // Only replicate the top-level basestore
      const stream = store1.replicate(true, { live: true })
      stream.pipe(store2.replicate(false, { live: true })).pipe(stream)
      return cb(null)
    },
...
async function create (storage, opts) {
  const store = new Basestore(storage, opts)
  await store.ready()
  return store
}
